...
def send_rent_notifications_task():
    ...
    db.session.flush()  # ensure log is written even if later operations fail
    ...
    for reminder in reminders:
        try:
            ...
            if not lease:
                current_app.logger.warning(f"Skipping reminder {reminder.id} - lease not found.")
                continue
            ...
            if not tenant or not property_ or not landlord:
                current_app.logger.warning(f"Skipping reminder {reminder.id} - missing tenant/property/landlord.")
                continue
            ...
            if already_paid:
                current_app.logger.info(f"Skipping tenant {tenant.name} - rent already paid for this month.")
            else:
                # Optionally auto-generate pending payment
                new_payment = RentPayment(
                    tenant_id=tenant.id,
                    property_id=property_.id,
                    amount=property_.monthly_rent,
                    payment_date=today,  # ensure this matches RentPayment column type
                    status="pending",
                    payment_mode="Cash"
                )
            ...
            tenant_msg = (
                f"Hello {tenant.name},\n\n"
                f"Your rent of Rs {property_.monthly_rent:.2f} is due on {reminder.due_date} "
                f"for {month}. Please pay on time.\n\nThanks!"
            )
            landlord_msg = (
            ...
            if tenant.email:
                try:
                    send_email_task.delay("Rent Payment Reminder", [tenant.email], tenant_msg, tenant_id=tenant.id)
                except Exception as e:
                    current_app.logger.error(f"Failed to queue tenant email for {tenant.email}: {e}", exc_info=True)
            ...
            if tenant.phone_number:
                try:
                    ...
                    current_app.logger.error(f"Failed to send SMS to tenant {tenant.phone_number}: {e}", exc_info=True)
                ...
                try:
                    ...
                    current_app.logger.error(f"Failed to send WhatsApp to tenant {tenant.phone_number}: {e}", exc_info=True)

            # Notify landlord
            if landlord.email:
                try:
                    send_email_task.delay("Tenant Rent Reminder Sent", [landlord.email], landlord_msg, tenant_id=tenant.id)
                except Exception as e:
                    current_app.logger.error(f"Failed to send landlord email to {landlord.email}: {e}", exc_info=True)

            if landlord.contact:
                try:
                    sid = twilio.send_sms(landlord.contact, landlord_msg, tenant_id=tenant.id, landlord_id=landlord.id)
                ...
                try:
                    ...
                    current_app.logger.error(f"Failed to send WhatsApp to landlord {landlord.contact}: {e}", exc_info=True)
            # Mark reminder as sent
            reminder.reminder_sent = True
            reminder.last_sent_date = today

        except Exception as inner_e:
            current_app.logger.error(f"Error processing reminder {reminder.id}: {inner_e}", exc_info=True)
...
def auto_end_expired_leases():
    ...
    try:
        ...
        if expired_leases:
            db.session.commit()
            print(f"{len(expired_leases)} leases updated to 'ended'.")
        else:
            print("No leases to update.")

    except Exception as e:
        db.session.rollback()
        print(f"Error updating expired leases: {e}")